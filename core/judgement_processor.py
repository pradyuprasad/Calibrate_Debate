...
class JudgementProcessor:
    ...
    def process_judgment(self, debate: DebateTotal, model: str) -> JudgeResult:
        ...
        try:
            judgment_string, usage = self.get_judgement_response(debate=debate, model=model)
            ...
            judge_result = self.extract_debate_result(xml_string=judgment_string, model=model)
