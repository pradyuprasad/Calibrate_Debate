...
class BetTournament:
   def __init__(self, config: Config):
       ...
       self.logger.info(f"Using {len(self.judge_models)} judges for evaluation: {', '.join(self.judge_models)}")
   ...
   def load_predefined_round1_matches(self) -> List[Dict]:
       ...
       try:
           ...
           if matches_file_path and Path(matches_file_path).exists():
               ...
               self.logger.info(f"Successfully loaded {len(round1_matches)} matches for round 1")
   ...
   def _generate_random_round1_pairings(self) -> List[Dict]:
       ...
       for i in range(0, len(models_list), 2):
           if i+1 < len(models_list):
               matches.append({
                   "prop_model": models_list[i],
                   "opp_model": models_list[i+1]
               })
   ...
   def generate_subsequent_round_pairings(self, round_num: int) -> List[Dict]:
       ...
       self.logger.info(f"Generating pairings for round {round_num} based on current standings")
       ...
       for i, model in enumerate(sorted_models):
           if i == 0 or (
               self.model_stats[model]["wins"] - self.model_stats[model]["losses"] ==
               self.model_stats[sorted_models[i-1]]["wins"] - self.model_stats[sorted_models[i-1]]["losses"]
           ):
       ...
       for i in range(0, len(unpaired), 2):
           if i+1 < len(unpaired):
               ...
               if unpaired[i+1] in self.model_stats[unpaired[i]]["opponents"]:
                   ...
                   if i+2 >= len(unpaired):
                       matches.append({
                           "prop_model": unpaired[i],
                           "opp_model": unpaired[i+1]
                       })
               else:
                   matches.append({
                       "prop_model": unpaired[i],
   ...
   def _find_winner(
        ...
        prop_confidence = sum(judge.confidence for judge in judgement_list if judge.winner == "proposition")
        opp_confidence = sum(judge.confidence for judge in judgement_list if judge.winner == "opposition")
        ...
        if prop_confidence > opp_confidence:
            ...
            self.logger.warning("Debate resulted in a tie in confidence - randomly selecting winner")
   ...
   def run_tournament(self):
       ...
       for round_num in range(1, self.num_rounds + 1):
           ...
           if len(round_matches) > len(self.topics):
                self.logger.warning(f"More matches ({len(round_matches)}) than available topics ({len(self.topics)}). Some topics will be reused.")
           ...
           for i, (match, topic) in enumerate(zip(round_matches, topics)):
               ...
               self.model_stats[match['prop_model']]["opponents"].append(match['opp_model'])
               self.model_stats[match['opp_model']]["opponents"].append(match['prop_model'])
               ...
               if not judgements:
                   self.logger.error(f"Failed to get judgements for debate: {debate_path}")
                   continue
