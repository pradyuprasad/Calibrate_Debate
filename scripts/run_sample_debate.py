...
def run_single_sample_debate(
    debate_service: DebateService,
    proposition_model: str,
    opposition_model: str,
    judge_models: List[str],
    path_to_use: Path,
    judgment_processor: JudgementProcessor,
    topic: DebateTopic,
) -> None:
    debate_obj = debate_service.run_debate(
        proposition_model=proposition_model,
        opposition_model=opposition_model,
        path_to_store=path_to_use,
...
def run_all_sample_debates() -> None:
    ...
    for idx, (proposition, opposition) in enumerate(pair_list):
        logging.info(
            f"running debate {idx + 1}/{len(pair_list)} with proposition {proposition} and opposition {opposition}"
        )
        ...
        path_to_use = (
            config.sample_debates_dir / f"{clean_prop_name}_{clean_opp_name}.json"
        )
