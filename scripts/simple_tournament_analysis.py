...
for debate_data in debates:
    ...
    if prop_bets["opening"] is not None and prop_bets["closing"] is not None:
        ...
        topic_confidence_changes[topic].append(change)
...
if topic_confidence_changes:
    ...
    sorted_topics = sorted(
        topic_confidence_changes.items(),
        key=lambda x: abs(sum(x[1])/len(x[1]) if x[1] else 0),
        reverse=True
    )

    for topic, changes in sorted_topics:
        if changes:
            avg_change = sum(changes) / len(changes)
            direction = "↑" if avg_change > 0 else "↓" if avg_change < 0 else "↔"
            print(f"Topic: {topic} - Avg change: {avg_change:.2f} {direction}")
