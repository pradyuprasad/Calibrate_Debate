...
class PrivateSelfTournamentRunnerInformed:
    def __init__(self, config: Config):
        self.config = config
        self.logger = setup_logging(config)

        # Create output directory for private self debates
        self.output_dir = Path("private_self_debates_informed")
    ...
    def run_debate(self, model: str, topic_idx: int) -> bool:
        ...
        try:
            debate = self.config.debate_service.run_debate(
                ...
                debate_type=DebateType.PRIVATE_SAME_DEBATOR_INFORMED
            )
...
def main():
    ...
    tournament = PrivateSelfTournamentRunnerInformed(config)
