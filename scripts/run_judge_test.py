...
def analyze_judge_consistency(sample_debates_list):
    ...
    for debate_path in sample_debates_list:
        ...
        prop_confidence = sum(j['confidence'] for j in debate_judgments if j['winner'] == 'proposition')
        opp_confidence = sum(j['confidence'] for j in debate_judgments if j['winner'] == 'opposition')

        # Store consensus data
        debate_consensus[debate_id] = {
            'vote_count': {'proposition': prop_votes, 'opposition': opp_votes},
            'confidence_weighted': {'proposition': prop_confidence, 'opposition': opp_confidence},
            'simple_consensus': 'proposition' if prop_votes > opp_votes else 'opposition',
            'weighted_consensus': 'proposition' if prop_confidence > opp_confidence else 'opposition',
    ...
    for model_name, votes in model_votes.items():
        ...
        avg_confidence_when_correct = confidence_correct_sum / (total_votes - disagreements) if (total_votes - disagreements) > 0 else 0
        avg_confidence_when_incorrect = confidence_incorrect_sum / disagreements if disagreements > 0 else 0

        # Calculate confidence-weighted agreement rate
        if total_confidence > 0:
            confidence_weighted_agreement = 1 - (confidence_incorrect_sum / total_confidence)
        else:
        ...
        model_consistency[model_name] = {
            ...
            'agreement_rate': 1 - (disagreements / total_votes) if total_votes > 0 else 0,
            ...
            'calibration_gap': avg_confidence_when_correct - avg_confidence_when_incorrect
        }

    # Identify outlier models (those with lowest agreement rates)
    sorted_models = sorted(
        model_consistency.items(),
    ...
    return {
        ...
        'outlier_models': sorted_models[:3] if len(sorted_models) >= 3 else sorted_models,
        'most_consistent_models': sorted_models[-3:][::-1] if len(sorted_models) >= 3 else sorted_models[::-1]
    }
...
for model, stats in sorted(results['model_consistency'].items(), key=lambda x: -x[1]['confidence_weighted_agreement']):
    print(f"  {model}:")
    print(f"    - Simple agreement rate: {stats['agreement_rate']:.2f} ({stats['total_votes']} votes, {stats['disagreements']} disagreements)")
    print(f"    - Confidence-weighted agreement: {stats['confidence_weighted_agreement']:.2f}")
    print(f"    - Avg confidence: {stats['avg_confidence']:.2f}")
    print(f"    - Avg confidence when correct: {stats['avg_confidence_when_correct']:.2f}")
    print(f"    - Avg confidence when incorrect: {stats['avg_confidence_when_incorrect']:.2f}")
...
for model, stats in results['outlier_models']:
    print(f"  {model}: {stats['confidence_weighted_agreement']:.2f} confidence-weighted agreement")
...
for debate_id, data in results['debate_consensus'].items():
    ...
    print(f"    - Weighted consensus: {weighted_winner} wins ({prop_conf:.2f}-{opp_conf:.2f})")
