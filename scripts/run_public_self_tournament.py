...
def setup_logging(config: Config):
    ...
    log_file = f"public_self_tournament_{timestamp}.log"
    config.logger.set_log_file(log_file)
    return logger


class PublicSelfTournamentRunner:
    ...
    def run_debate(self, model: str, topic_idx: int) -> bool:
        """Run a single self-debate with public betting."""
        ...
        try:
            debate = self.config.debate_service.run_debate(
                ...
                debate_type=DebateType.PUBLIC_SAME_DEBATOR
            )
    ...
    def run_tournament(self):
        ...
        self.logger.info("Starting public self-debate tournament")
...
def main():
    """Run the public self-debate tournament."""
    load_dotenv()
    config = Config()

    tournament = PublicSelfTournamentRunner(config)
